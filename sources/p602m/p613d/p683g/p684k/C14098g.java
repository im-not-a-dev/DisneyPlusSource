package p602m.p613d.p683g.p684k;

import java.io.IOException;
import java.io.OutputStream;

/* renamed from: m.d.g.k.g */
/* compiled from: HexEncoder */
public class C14098g implements C14095d {

    /* renamed from: a */
    protected final byte[] f31119a = {48, Framer.STDOUT_FRAME_PREFIX, Framer.STDERR_FRAME_PREFIX, 51, 52, 53, 54, 55, 56, 57, 97, 98, 99, 100, 101, 102};

    /* renamed from: b */
    protected final byte[] f31120b = new byte[128];

    public C14098g() {
        mo35665a();
    }

    /* renamed from: a */
    private static boolean m44512a(char c) {
        return c == 10 || c == 13 || c == 9 || c == ' ';
    }

    /* access modifiers changed from: protected */
    /* renamed from: a */
    public void mo35665a() {
        int i = 0;
        int i2 = 0;
        while (true) {
            byte[] bArr = this.f31120b;
            if (i2 >= bArr.length) {
                break;
            }
            bArr[i2] = -1;
            i2++;
        }
        while (true) {
            byte[] bArr2 = this.f31119a;
            if (i < bArr2.length) {
                this.f31120b[bArr2[i]] = (byte) i;
                i++;
            } else {
                byte[] bArr3 = this.f31120b;
                bArr3[65] = bArr3[97];
                bArr3[66] = bArr3[98];
                bArr3[67] = bArr3[99];
                bArr3[68] = bArr3[100];
                bArr3[69] = bArr3[101];
                bArr3[70] = bArr3[102];
                return;
            }
        }
    }

    /* renamed from: b */
    public int mo35662b(byte[] bArr, int i, int i2, OutputStream outputStream) throws IOException {
        int i3 = i2 + i;
        while (i3 > i && m44512a((char) bArr[i3 - 1])) {
            i3--;
        }
        int i4 = 0;
        while (i < i3) {
            while (i < i3 && m44512a((char) bArr[i])) {
                i++;
            }
            int i5 = i + 1;
            byte b = this.f31120b[bArr[i]];
            while (i5 < i3 && m44512a((char) bArr[i5])) {
                i5++;
            }
            int i6 = i5 + 1;
            byte b2 = this.f31120b[bArr[i5]];
            if ((b | b2) >= 0) {
                outputStream.write((b << 4) | b2);
                i4++;
                i = i6;
            } else {
                throw new IOException("invalid characters encountered in Hex data");
            }
        }
        return i4;
    }

    /* renamed from: a */
    public int mo35660a(byte[] bArr, int i, int i2, OutputStream outputStream) throws IOException {
        for (int i3 = i; i3 < i + i2; i3++) {
            byte b = bArr[i3] & 255;
            outputStream.write(this.f31119a[b >>> 4]);
            outputStream.write(this.f31119a[b & 15]);
        }
        return i2 * 2;
    }

    /* renamed from: a */
    public int mo35659a(String str, OutputStream outputStream) throws IOException {
        int length = str.length();
        while (length > 0 && m44512a(str.charAt(length - 1))) {
            length--;
        }
        int i = 0;
        int i2 = 0;
        while (i < length) {
            while (i < length && m44512a(str.charAt(i))) {
                i++;
            }
            int i3 = i + 1;
            byte b = this.f31120b[str.charAt(i)];
            while (i3 < length && m44512a(str.charAt(i3))) {
                i3++;
            }
            int i4 = i3 + 1;
            byte b2 = this.f31120b[str.charAt(i3)];
            if ((b | b2) >= 0) {
                outputStream.write((b << 4) | b2);
                i2++;
                i = i4;
            } else {
                throw new IOException("invalid characters encountered in Hex string");
            }
        }
        return i2;
    }
}
